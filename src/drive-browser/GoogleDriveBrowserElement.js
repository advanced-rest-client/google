 ...
 */
export default class GoogleDriveBrowserElement extends EventsTargetMixin(LitElement) {
  static get styles() {
  ...
  [headerTemplate]() {
    ...
    <header>
      ...
      <anypoint-button data-action="refresh-list" @click="${this.refresh}" ?anypoint="${anypoint}">Refresh</anypoint-button>
    </header>
    `;
  }
   ...
   */
  [searchTemplate]() {
    const { anypoint, query, outlined } = this;
    return html`
    <div class="search">
      <anypoint-input
        role="textbox"
   ...
   */
  [loadNextTemplate]() {
    const { hasMore, anypoint } = this;
    if (!hasMore) {
      return '';
    }
    return html`
    <anypoint-button ?anypoint="${anypoint}" @click="${this.queryNext}" ?disabled="${this.loading}">Load more</anypoint-button>
    `;
  }
}
